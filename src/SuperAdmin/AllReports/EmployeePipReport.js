...
//   const [selectedDivision, setSelectedDivision] = useState(""); // Holds division_id
//   const [searchTerm, setSearchTerm] = useState("");
//   const [page, setPage] = useState(0);
//   const [rowsPerPage, setRowsPerPage] = useState(10);

//   // Fetch the list of divisions when the component mounts
//   useEffect(() => {
...
//         setError("Could not load division list.");
//       }
//     };
//     fetchDivisions();
//   }, []); // Empty dependency array means this runs only once

//   // Fetch the report data when the "Generate Report" button is clicked
//   const handleFetchReport = async () => {
//     if (!selectedDivision) {
//       setError("Please select a division to generate the report.");
//       return;
//     }
//     setLoading(true);
//     setError(null);
...
//         `/apis/get_employee_pip_report_get_report/?year=${selectedYear}&division_id=${selectedDivision}`
//       );
//       const rawData = Array.isArray(response.data.results) ? response.data.results : [];
//       setReportData(rawData);
//     } catch (err) {
//       setError("Failed to fetch PIP report. Please try again later.");
//       console.error(err);
//     } finally {
//       setLoading(false);
//     }
...
//   // Export the full (unpaginated) filtered data to an Excel file
//   const handleExport = () => {
...
//       "PIP Start Date": formatDate(row["PIP Start Date"]),
//       "PIP End Date": formatDate(row["PIP End Date"]),
//       "TMS Score": row["TMS Score"],
//       "LM Score": row["LM Score"],
//       "HOD Score": row["HOD Score"],
//       "HR Score": row["HR Score"],
//       "Date": formatDate(row["Date"]),
//     }));

//     const worksheet = XLSX.utils.json_to_sheet(dataToExport);
//     const workbook = XLSX.utils.book_new();
//     XLSX.utils.book_append_sheet(workbook, worksheet, "PIP_Report");
//     XLSX.writeFile(workbook, `PIP_Report_${selectedYear}.xlsx`);
//   };
...
//   // Define columns based on the API response keys
//   const columns = [
...
//     { id: "PIP Start Date", label: "PIP Start Date" },
//     { id: "PIP End Date", label: "PIP End Date" },
//     { id: "TMS Score", label: "TMS Score" },
//     { id: "LM Score", label: "LM Score" },
//     { id: "HOD Score", label: "HOD Score" },
//     { id: "HR Score", label: "HR Score" },
//     { id: "Date", label: "Date" },
//   ];
...
//               {divisions.map((div) => (
//                 <MenuItem key={div.division_id} value={div.division_id}>{div.division_name}</MenuItem>
//               ))}
//             </Select>
//           </FormControl>
...
//             <TableRow>
//               {columns.map((col) => (
//                 <TableCell key={col.id} sx={{ backgroundColor: "#e3f2fd", fontWeight: 600 }}>{col.label}</TableCell>
//               ))}
//             </TableRow>
//           </TableHead>
//           <TableBody>
...
//             ) : !hasSearched ? (
//               <TableRow><TableCell colSpan={columns.length} align="center">Please select a year and division, then click Generate Report.</TableCell></TableRow>
...
//                   <TableCell>{formatDate(row["PIP Start Date"])}</TableCell>
//                   <TableCell>{formatDate(row["PIP End Date"])}</TableCell>
//                   <TableCell>{row["TMS Score"] ?? "N/A"}</TableCell>
//                   <TableCell>{row["LM Score"] ?? "N/A"}</TableCell>
//                   <TableCell>{row["HOD Score"] ?? "N/A"}</TableCell>
//                   <TableCell>{row["HR Score"] ?? "N/A"}</TableCell>
...
// export default EmployeePipReport;    //// 
...
//   const handleFetchReport = async () => {
//     if (!selectedFinancialYear || !selectedDivision) {
//       setError("Please select both a Financial Year and a Division.");
//       setHasSearched(true);
//       return;
//     }
//     setLoading(true);
...
//     try {
//       const response = await axiosInstance.get(
//         `/apis/get_employee_pip_report_get_report/?year=${selectedFinancialYear}&division_id=${selectedDivision}`
//       );
//       const rawData = Array.isArray(response.data.results) ? response.data.results : [];
//       setReportData(rawData);
//     } catch (err) {
...
//       "Designation": row.designation_name,
//       "Division": row.division_name,
//       "Sub Division": row.sub_division, // Corrected key from sub_division_name
//       "Level": row.level,
...
//     const workbook = XLSX.utils.book_new();
//     XLSX.utils.book_append_sheet(workbook, worksheet, "PIP_Report");
//     XLSX.writeFile(workbook, `PIP_Report_${selectedFinancialYear}.xlsx`);
//   };
...
//   // Mapped all columns based on the new JSON structure
//   const columns = [
//     { id: "sr_no", label: "SR. NO." },
//     { id: "employee_id", label: "Employee ID" },
//     { id: "employee_name", label: "Employee Name" },
...
//     { id: "manager_name", label: "Manager" },
//     { id: "date_of_joining", label: "Joining Date" },
//     { id: "pip_start_date", label: "PIP Start Date" },
//     { id: "pip_end_date", label: "PIP End Date" },
//     { id: "tms_score", label: "TMS Score" },
//     { id: "lm_score", label: "LM Score" },
//     { id: "hod_score", label: "HOD Score" },
//     { id: "hr_score", label: "HR Score" },
//     { id: "date", label: "Date" },
//   ];
...
//             disabled={!selectedFinancialYear || !selectedDivision || loading}
//           >
//             Generate Report
//           </Button>
...
//               paginatedData.map((row, index) => (
//                 <TableRow key={row.user_id} sx={{ "&:nth-of-type(odd)": { backgroundColor: "#f9f9f9" } }}>
//                   {/* Correctly mapped all TableCell data points to the JSON response */}
//                   <TableCell>{page * rowsPerPage + index + 1}</TableCell>
//                   <TableCell>{row.employee_id ?? "N/A"}</TableCell>
//                   <TableCell>{row.employee_name ?? "N/A"}</TableCell>
//                   <TableCell>{row.department_name ?? "N/A"}</TableCell>
...
const EmployeePipReport = () => {
  ...
  useEffect(() => {
    const fetchDivisions = async () => {
      try {
        const response = await axiosInstance.get("/api/division/");
        if (Array.isArray(response.data)) setDivisions(response.data);
      } catch (err) {
        console.error("Failed to fetch divisions:", err);
        setError("Could not load division list.");
      }
  ...
  const handleFetchReport = async () => {
    ...
    setReportData([]);
    try {
      const response = await axiosInstance.get(`/apis/get_employee_pip_report_get_report/?year=${selectedFinancialYear}&division_id=${selectedDivision}`);
      const rawData = Array.isArray(response.data.results) ? response.data.results : [];
      setReportData(rawData);
    } catch (err) {
      setError("Failed to fetch PIP report. Please try again later.");
  ...
  const handleExportExcel = () => {
    const dataToExport = filteredData.map((row, index) => ({
      ...
      "Manager": row.manager_name, "Joining Date": formatDate(row.date_of_joining),
      "PIP Start Date": formatDate(row["PIP Start Date"]), "PIP End Date": formatDate(row["PIP End Date"]),
      "TMS Score": row["TMS Score"], "LM Score": row["LM Score"], "HOD Score": row["HOD Score"],
      "HR Score": row["HR Score"], "Date": formatDate(row["Date"]),
    }));
    const worksheet = XLSX.utils.json_to_sheet(dataToExport);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "PIP_Report");
  ...
  const columns = [
    ...
    "Sub Division", "Level", "Headquarter", "Manager", "Joining Date", "PIP Start Date",
    "PIP End Date", "TMS Score", "LM Score", "HOD Score", "HR Score", "Date",
  ];
      ...
      <Paper sx={{ p: 2, mb: 2 }}>
        <Stack direction="row" spacing={2} mb={2} flexWrap="wrap">
          ...
          <FormControl sx={{ minWidth: 180 }} size="small">
            <InputLabel>Division</InputLabel>
            <Select value={selectedDivision} label="Division" onChange={(e) => setSelectedDivision(e.target.value)}>
              {divisions.map((div) => <MenuItem key={div.division_id} value={div.division_id}>{div.division_name}</MenuItem>)}
            </Select>
          </FormControl>
          <Button variant="contained" onClick={handleFetchReport} disabled={!selectedFinancialYear || !selectedDivision || loading}
        ...
        <Table stickyHeader size="small">
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow><TableCell colSpan={columns.length} align="center">Please select filters and click Generate.</TableCell></TableRow>
            ) : paginatedData.length > 0 ? (
              paginatedData.map((row, index) => (
                <TableRow key={row.user_id || index} hover>
                  ...
                  <TableCell>{formatDate(row["PIP Start Date"])}</TableCell>
                  <TableCell>{formatDate(row["PIP End Date"])}</TableCell>
                  <TableCell align="center">{row["TMS Score"] ?? "N/A"}</TableCell>
                  <TableCell align="center">{row["LM Score"] ?? "N/A"}</TableCell>
                  <TableCell align="center">{row["HOD Score"] ?? "N/A"}</TableCell>
                  <TableCell align="center">{row["HR Score"] ?? "N/A"}</TableCell>
                  <TableCell>{formatDate(row["Date"])}</TableCell>
                </TableRow>
              ))
            ) : (
