...
// // Helper function to format document keys into readable names
...
//     // Don't fetch if no user ID is present
//     if (!employeeId) {
//       setLoading(false);
//       return;
//     }
...
//     try {
//       // --- STEP 1: Fetch the user's email from the first API endpoint ---
//       const detailsResponse = await axiosInstance.get(`/api/document_details/?user_id=${employeeId}`);
      
//       if (detailsResponse.data.status !== 'success' || !detailsResponse.data.email) {
//         throw new Error("Could not retrieve user email to fetch documents.");
//       }
      
//       const userEmail = detailsResponse.data.email;

//       // --- STEP 2: Use the fetched email to get the list of pre-defined documents ---
//       const documentsResponse = await axiosInstance.post('/fetch_documents/', {
//         email_id: userEmail,
//       });

//       if (documentsResponse.data.status === 'success' && documentsResponse.data.documents) {
//         // Transform the response object into an array for easier rendering
//         const fetchedDocs = documentsResponse.data.documents;
//         const formattedDocs = Object.entries(fetchedDocs).map(([key, url]) => ({
//           name: formatDocumentName(key), // e.g., "passport_photo" -> "Passport Photo"
//           url: url, // The URL can be null
//         }));
//         setDocuments(formattedDocs);
//       } else {
//         setDocuments([]); // Set to empty if the 'documents' key is missing
//       }
//     } catch (err) {
...
const Documents = ({ onNext, onBack }) => {
  ...
  const fetchPredefinedDocuments = useCallback(async () => {
    if (!userId) { setIsLoadingPredefined(false); return; };
    setIsLoadingPredefined(true);
    try {
      const detailsResponse = await axiosInstance.get(`/api/document_details/?user_id=${userId}`);
      if (detailsResponse.data.status !== 'success' || !detailsResponse.data.email) throw new Error("Could not retrieve user email.");
      const userEmail = detailsResponse.data.email;
    ...
    } catch (error) { console.error(error); } finally { setIsLoadingPredefined(false); }
  }, [userId]);
  ...
  const validateForm = () => {
      ...
      if (!form.type.trim()) { newErrors.type = true; isValid = false; }
      if (!editMode && !form.file) { newErrors.file = true; isValid = false; }
  ...
  const handleFinish = () => {
      // Validate that at least one document exists before finishing
  ...
  return (
    <Box p={2}>
      <Typography variant="h6" gutterBottom color={PRIMARY_COLOR} fontWeight="bold">Manage Uploaded Documents</Typography>
      ...
      <TableContainer component={Paper} sx={{ mb: 4 }}>
        <Table size="small">
          ...
          <TableBody>
            {uploadedDocuments.length === 0 ? (
                <TableRow><TableCell colSpan={4} align="center">No documents uploaded yet.</TableCell></TableRow>
            ) : (
                uploadedDocuments.map((doc) => (
                    <TableRow key={doc.id}>
                        <TableCell>{doc.name}</TableCell>
                        <TableCell>{doc.type}</TableCell>
                        <TableCell>
                          <Button variant="text" size="small" href={doc.fileUrl} target="_blank" rel="noopener noreferrer"><Description fontSize="small" sx={{ mr: 0.5 }} /> View</Button>
                        </TableCell>
                        <TableCell align="right">
                          <IconButton title="Edit" size="small" color="primary" onClick={() => handleEdit(doc)}><Edit fontSize="small" /></IconButton>
                          <IconButton title="Delete" size="small" color="error" onClick={() => handleDelete(doc.id)}><Delete fontSize="small" /></IconButton>
      ...
      <Box mb={4} p={3} sx={{ border: '1px dashed #ccc', borderRadius: 2, bgcolor: '#fafafa' }}>
        ...
        <Box display="flex" gap={2} flexWrap="wrap" alignItems="flex-start" mt={1}>
          <TextField 
            label="Document Name" name="name" size="small" 
            value={form.name} onChange={handleChange} error={!!errors.name} sx={{ minWidth: 200 }} 
          />
          <TextField 
            label="Document Type" name="type" size="small" 
            value={form.type} onChange={handleChange} error={!!errors.type} sx={{ minWidth: 200 }} 
          />
          
          <Box>
            <Button 
                variant="outlined" component="label" size="medium" 
                sx={{ textTransform: 'none', height: 40, borderColor: errors.file ? 'red' : 'inherit' }}
            >
                {form.fileName || 'Choose file...'}
                <input type="file" name="file" accept=".png,.jpg,.jpeg,.pdf,.docx" hidden onChange={handleChange} />
            </Button>
          </Box>

          <Button variant="contained" onClick={handleSubmit} sx={{ bgcolor: PRIMARY_COLOR, height: 40 }}>{editMode ? 'Update' : 'Upload'}</Button>
          {editMode && (<Button variant="text" onClick={resetForm} sx={{ height: 40 }}>Cancel</Button>)}
        </Box>
      </Box>
      ...
      <Typography variant="h6" gutterBottom color={PRIMARY_COLOR} fontWeight="bold">Company & Statutory Documents</Typography>
      <TableContainer component={Paper} sx={{ mb: 4 }}>
            <Table size="small">
                <TableHead sx={{ bgcolor: '#f5f5f5' }}>
                    <TableRow><TableCell><strong>DOCUMENT TYPE</strong></TableCell><TableCell align="right"><strong>ACTION</strong></TableCell></TableRow>
                </TableHead>
                <TableBody>
                    {isLoadingPredefined ? (<TableRow><TableCell colSpan={2} align="center"><CircularProgress size={24} /></TableCell></TableRow>) 
                    : predefinedDocuments.length === 0 ? (<TableRow><TableCell colSpan={2} align="center">No pre-defined documents found.</TableCell></TableRow>) 
                    : (predefinedDocuments.map((doc, index) => (
                            <TableRow key={index}>
                                <TableCell>{doc.name}</TableCell>
                                <TableCell align="right">
                                    {doc.url ? (<Button variant="contained" size="small" href={doc.url} target="_blank" sx={{ bgcolor: PRIMARY_COLOR }}>View / Download</Button>) 
                                    : (<Typography variant="caption" color="text.secondary">Not Available</Typography>)}
                                </TableCell>
                            </TableRow>
                        ))
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 4, pt: 2, borderTop: '1px solid #eee' }}>
        <Button onClick={onBack} variant="outlined" size="large" sx={{ borderRadius: '8px', borderColor: '#ccc', color: '#555', '&:hover': { borderColor: '#8C257C', color: '#8C257C' } }}>Back</Button>
        <Button onClick={handleFinish} variant="contained" size="large" sx={{ background: `linear-gradient(135deg, ${PRIMARY_COLOR} 0%, #6d1d60 100%)`, color: 'white', borderRadius: '8px' }}>Finish & Generate PDF</Button>
      </Box>
    </Box>
  );
};
