...
const Allowances = () => {
  ...
  const [allowances, setAllowances] = useState([]);
  const [editingId, setEditingId] = useState(null);
  // 3. Make 10 entries to show Standardized
  ...
  const tableHeaderStyles = {
    color: 'white',
    fontWeight: 'bold',
    borderRight: '1px solid rgba(224, 224, 224, 0.2)', // Optional: for cell separation
    '&:last-child': {
      borderRight: 0,
    }
  };
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      {/* Top Controls: Rows per page and Search Bar */}
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
        <FormControl size="small" sx={{ minWidth: 80 }}>
      ...
      {/* Bottom Controls: Total Rows and Pagination */}
          ...
          <Box display="flex" justifyContent="flex-end">
            <Button
              ...
              sx={{
                ...
                '&:hover': {
                  backgroundColor: '#6d1b60' // Darker shade of purple for hover
                }
              }}
              onClick={handleSave}
            >
