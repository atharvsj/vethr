...
// }
  ...
  //     } catch (error) {
  //       console.error("Error submitting feedback:", error);
  //       if (error.response && error.response.data && error.response.data.message === "You have already answered the questionnaire.") {
  //         Swal.fire({
  //           icon: 'info',
  //           title: 'Already Submitted',
  //           text: 'Questionnaire already submitted.',
  //         });
  //       } else {
  //         Swal.fire({
  //           icon: 'error',
  ...
  //               backgroundColor: "#9900cc",
  //             },
  //             boxShadow: clickedSubmit ? `0 0 10px 3px ${electricPurple}` : "none",
  //             transition: "box-shadow 0.3s ease-in-out",
  //           }}
  //         >
  //           Submit
...
//       try {
//         const [questionsResponse, employeeStatusResponse] = await Promise.all([
//           axiosInstance.get("exit-questionnaire/"),
...
//         if (employeeStatusResponse.data) {
//           const status = employeeStatusResponse.data.exit_interview_questionnaire;
//           if (status === "Y") {
...
//         Swal.fire('Error', 'Failed to load questionnaire data. Please try again.', 'error');
//         setIsFormDisabled(true);
//       } finally {
//         setLoading(false);
//       }
...
//     } catch (error) {
//       console.error("Error submitting feedback:", error);
//       if (error.response?.data?.message === "You have already answered the questionnaire.") {
//         Swal.fire({
//           icon: 'info',
//           title: 'Already Submitted',
//           text: 'Questionnaire already submitted.',
...
//   return (
//     <Card sx={{ mt: 3, height: "80vh", display: "flex", flexDirection: "column" }}>
//       <CardContent sx={{ flex: "0 0 auto", px: 2 }}>
//         <Typography variant="h5" fontWeight="bold" color="#8C257C" gutterBottom>
//           EXIT INTERVIEW QUESTIONNAIRE
//         </Typography>
//       </CardContent>
...
//         p: 2,
//         display: 'flex',
//         justifyContent: 'space-between', // Aligns items to the ends
//         alignItems: 'center' // Vertically centers items
//       }}>
//         {/* Conditionally render the read-only message */}
//         {isFormDisabled && (
//           <Typography variant="body2" color="text.secondary" sx={{ fontStyle: 'italic' }}>
//             This questionnaire has already been submitted and is now in read-only mode.
//           </Typography>
//         )}
        
//         {/* Spacer to push the button to the right if the message isn't visible */}
//         {!isFormDisabled && <Box sx={{ flexGrow: 1 }} />}
  ...
  useEffect(() => {
    ...
    const fetchInitialData = async () => {
      try {
        ...
        if (employeeStatusResponse.data) {
          const statusArray = employeeStatusResponse.data.exit_interview_questionnaire;
          if (Array.isArray(statusArray) && statusArray[0] === "Y") {
            setIsFormDisabled(true);
          } else {
            setIsFormDisabled(false);
          }
