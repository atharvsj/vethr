...
// export default function HolidayViewLM() {
//   // State for API data, loading, and errors
//   const [holidays, setHolidays] = useState([]);
//   const [loading, setLoading] = useState(true);
//   const [error, setError] = useState(null);
...
//   useEffect(() => {
//     const fetchHolidays = async () => {
//       // Get employeeId from localStorage, with a fallback for testing
//       const employeeId = localStorage.getItem('loggedInUser');
...
//             setHolidays(formattedData);
//         } else {
//             // This error will be thrown if the response is not an array
...
export default function HolidayViewLM() {
  ...
  // State for table controls
  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(10); // Replaces 'entries'
  const [searchTerm, setSearchTerm] = useState('');
  
  // --- UI & Theme Constants as per requirements ---
  ...
  const filteredData = holidays.filter(holiday =>
    holiday.holiday.toLowerCase().includes(searchTerm.toLowerCase()) ||
    formatDate(holiday.startDate).includes(searchTerm) ||
    formatDate(holiday.endDate).includes(searchTerm) ||
    holiday.type.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  // Sliced data for the current page
  const paginatedData = filteredData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage);
  const totalColumns = 5; // Updated for Sr. No.
  ...
  return (
    <Box component={Paper} sx={{ p: 3 }}>
      ...
      <Box
        sx={{
          display: 'flex',
          justifyContent: 'flex-end', // Aligned to right as there is no "Add" button
          alignItems: 'center',
          mb: 2,
      ...
      <TableContainer>
        <Table sx={{ minWidth: '100%', whiteSpace: 'nowrap' }}>
          ...
          <TableBody>
            {loading ? (
              // --- Skeleton Loader as per UI requirements ---
              ...
              paginatedData.map((holiday, index) => (
                <TableRow hover key={holiday.id}>
                  {/* --- Sr. No. cell with correct calculation for pagination --- */}
      ...
      {/* --- MUI TablePagination as per UI requirements --- */}
      <TablePagination
