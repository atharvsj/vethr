...
export default function TasksView() {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        const res = await axios.get("https://tdtlworld.com/hrms-backend/tasks/");
  ...
  const taskStats = [
    {
      ...
      count: tasks.filter((task) => task.task_status === "2").length,
      color: "success.main",
    },
    {
      title: "TOTAL IN PROGRESS",
      count: tasks.filter((task) => task.task_status === "1").length,
      color: "primary.main",
    },
    {
      title: "TOTAL NOT STARTED",
      count: tasks.filter((task) => task.task_status === "0").length,
      color: "warning.main",
    },
    {
      title: "TOTAL ON HOLD",
      count: tasks.filter((task) => task.task_status === "3").length,
      color: "error.main",
    },
  ];
              ...
              {filteredTasks.length > 0 ? (
                filteredTasks.slice(0, Number(entries)).map((task) => (
                  <TableRow key={task.task_id}>
                    ...
                    <TableCell>
                      {statusMap[Number(task.task_status)] || "Unknown"}
                    </TableCell>
