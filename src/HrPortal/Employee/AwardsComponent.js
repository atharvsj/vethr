...
const columns = [
  ...
  { key: 'awardGift', label: 'AWARD GIFT' },
  { key: 'awardCash', label: 'AWARD CASH' },
  { key: 'monthYear', label: 'MONTH & YEAR' },
];

const initialData = []; // Add award data here
...
const AwardsComponent = () => {
  ...
  return (
    <Box>
      <Typography variant="subtitle1" sx={{ mb: 1, ml: 1 }}>List All Awards</Typography>
            ...
            {filteredData.length > 0 ? (
              filteredData.slice(0, entries).map((row, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell>{row.awardGift}</TableCell>
                  <TableCell>{row.awardCash}</TableCell>
