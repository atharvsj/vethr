...
import {
  ...
  Avatar, IconButton, Tooltip, Button
} from '@mui/material';
import {
...
const columns = [
  ...
  { key: 'arrangementType', label: 'ARRANGEMENT TYPE' },
  { key: 'actualBudget', label: 'ACTUAL BUDGET' },
  { key: 'endDate', label: 'END DATE' },
];

const initialData = []; // Add travel request objects here if needed
...
const TravelRequestComponent = () => {
  ...
  return (
    <Box>
      <Typography variant="subtitle1" sx={{ mb: 1, ml: 1 }}>List All Travel Request</Typography>
            ...
            {filteredData.length > 0 ? (
              filteredData.slice(0, entries).map((row, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell>{row.placeOfVisit}</TableCell>
                  <TableCell>{row.visitPurpose}</TableCell>
                  <TableCell>{row.arrangementType}</TableCell>
                  <TableCell>{row.actualBudget}</TableCell>
